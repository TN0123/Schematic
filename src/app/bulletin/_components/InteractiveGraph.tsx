"use client";

import React, { useCallback, useRef, useEffect, useState } from "react";
import ForceGraph2D from "react-force-graph-2d";
import { Plus, Trash2, Link, Share2 } from "lucide-react";

export interface GraphNode {
  id: string;
  name: string;
  color?: string;
  size?: number;
  x?: number;
  y?: number;
}

export interface GraphLink {
  source: string;
  target: string;
  label?: string;
  color?: string;
  width?: number;
}

export interface GraphData {
  nodes: GraphNode[];
  links: GraphLink[];
}

interface InteractiveGraphProps {
  data: GraphData;
  onChange: (data: GraphData) => void;
  height?: number;
}

export default function InteractiveGraph({
  data,
  onChange,
  height = 400,
}: InteractiveGraphProps) {
  const graphRef = useRef<any>(null);
  const [selectedNode, setSelectedNode] = useState<string | null>(null);
  const [isCreatingLink, setIsCreatingLink] = useState(false);
  const [linkSource, setLinkSource] = useState<string | null>(null);
  const [newNodeName, setNewNodeName] = useState("");
  const [showAddNode, setShowAddNode] = useState(false);

  // Initialize with default data if empty
  const graphData =
    !data || !data.nodes || data.nodes.length === 0
      ? {
          nodes: [
            { id: "1", name: "Central Idea", color: "#10b981", size: 8 },
            { id: "2", name: "Sub Topic 1", color: "#3b82f6", size: 6 },
            { id: "3", name: "Sub Topic 2", color: "#8b5cf6", size: 6 },
          ],
          links: [
            { source: "1", target: "2", label: "relates to" },
            { source: "1", target: "3", label: "connects to" },
          ],
        }
      : data;

  const handleNodeClick = useCallback(
    (node: GraphNode) => {
      if (isCreatingLink) {
        if (linkSource && linkSource !== node.id) {
          // Create new link
          const newLink: GraphLink = {
            source: linkSource,
            target: node.id,
            label: "connects to",
            color: "#6b7280",
          };

          const updatedData = {
            ...graphData,
            links: [...graphData.links, newLink],
          };
          onChange(updatedData);
          setIsCreatingLink(false);
          setLinkSource(null);
        } else {
          setLinkSource(node.id);
        }
      } else {
        setSelectedNode(selectedNode === node.id ? null : node.id);
      }
    },
    [isCreatingLink, linkSource, selectedNode, graphData, onChange]
  );

  const handleNodeRightClick = useCallback(
    (node: GraphNode) => {
      if (confirm(`Delete node "${node.name}"?`)) {
        const updatedData = {
          nodes: graphData.nodes.filter((n) => n.id !== node.id),
          links: graphData.links.filter(
            (l) => l.source !== node.id && l.target !== node.id
          ),
        };
        onChange(updatedData);
        setSelectedNode(null);
      }
    },
    [graphData, onChange]
  );

  const addNode = () => {
    if (!newNodeName.trim()) return;

    const newNode: GraphNode = {
      id: Date.now().toString(),
      name: newNodeName.trim(),
      color: `#${Math.floor(Math.random() * 16777215).toString(16)}`,
      size: 6,
    };

    const updatedData = {
      ...graphData,
      nodes: [...graphData.nodes, newNode],
    };
    onChange(updatedData);
    setNewNodeName("");
    setShowAddNode(false);
  };

  const updateNodeName = (nodeId: string, newName: string) => {
    const updatedData = {
      ...graphData,
      nodes: graphData.nodes.map((node) =>
        node.id === nodeId ? { ...node, name: newName } : node
      ),
    };
    onChange(updatedData);
  };

  const deleteSelectedNode = () => {
    if (!selectedNode) return;

    const node = graphData.nodes.find((n) => n.id === selectedNode);
    if (node && confirm(`Delete node "${node.name}"?`)) {
      const updatedData = {
        nodes: graphData.nodes.filter((n) => n.id !== selectedNode),
        links: graphData.links.filter(
          (l) => l.source !== selectedNode && l.target !== selectedNode
        ),
      };
      onChange(updatedData);
      setSelectedNode(null);
    }
  };

  return (
    <div className="border dark:border-dark-divider rounded-lg overflow-hidden">
      {/* Control Panel */}
      <div className="bg-gray-50 dark:bg-dark-secondary border-b dark:border-dark-divider p-3">
        <div className="flex items-center gap-2 mb-3">
          <button
            onClick={() => setShowAddNode(!showAddNode)}
            className="flex items-center gap-1 px-3 py-1 bg-green-500 text-white rounded text-sm hover:bg-green-600 transition-colors"
          >
            <Plus className="w-4 h-4" />
            Add Node
          </button>
          <button
            onClick={() => setIsCreatingLink(!isCreatingLink)}
            className={`flex items-center gap-1 px-3 py-1 rounded text-sm transition-colors ${
              isCreatingLink
                ? "bg-blue-500 text-white hover:bg-blue-600"
                : "bg-gray-200 dark:bg-dark-tertiary text-gray-700 dark:text-dark-textSecondary hover:bg-gray-300 dark:hover:bg-gray-600"
            }`}
          >
            <Link className="w-4 h-4" />
            {isCreatingLink ? "Cancel Link" : "Create Link"}
          </button>
          {selectedNode && (
            <button
              onClick={deleteSelectedNode}
              className="flex items-center gap-1 px-3 py-1 bg-red-500 text-white rounded text-sm hover:bg-red-600 transition-colors"
            >
              <Trash2 className="w-4 h-4" />
              Delete
            </button>
          )}
        </div>

        {showAddNode && (
          <div className="flex gap-2">
            <input
              type="text"
              value={newNodeName}
              onChange={(e) => setNewNodeName(e.target.value)}
              placeholder="Node name..."
              className="flex-1 px-2 py-1 border dark:border-dark-divider rounded text-sm dark:bg-dark-tertiary dark:text-dark-textPrimary"
              onKeyDown={(e) => e.key === "Enter" && addNode()}
              autoFocus
            />
            <button
              onClick={addNode}
              className="px-3 py-1 bg-green-500 text-white rounded text-sm hover:bg-green-600 transition-colors"
            >
              Add
            </button>
          </div>
        )}

        {isCreatingLink && linkSource && (
          <div className="text-sm text-blue-600 dark:text-blue-400">
            Click another node to create a link from "
            {graphData.nodes.find((n) => n.id === linkSource)?.name}"
          </div>
        )}
      </div>

      {/* Graph Visualization */}
      <div style={{ height }}>
        <ForceGraph2D
          ref={graphRef}
          graphData={graphData}
          nodeLabel="name"
          nodeCanvasObject={(
            node: any,
            ctx: CanvasRenderingContext2D,
            globalScale: number
          ) => {
            const label = node.name;
            const fontSize = 12 / globalScale;
            ctx.font = `${fontSize}px Sans-Serif`;
            const textWidth = ctx.measureText(label).width;
            const bckgDimensions = [textWidth, fontSize].map(
              (n) => n + fontSize * 0.2
            );

            // Draw node circle
            ctx.beginPath();
            ctx.arc(node.x, node.y, node.size || 6, 0, 2 * Math.PI, false);
            ctx.fillStyle = node.color || "#10b981";
            ctx.fill();

            // Draw selection ring
            if (selectedNode === node.id) {
              ctx.beginPath();
              ctx.arc(
                node.x,
                node.y,
                (node.size || 6) + 2,
                0,
                2 * Math.PI,
                false
              );
              ctx.strokeStyle = "#3b82f6";
              ctx.lineWidth = 2;
              ctx.stroke();
            }

            // Draw label background
            ctx.fillStyle = "rgba(255, 255, 255, 0.8)";
            ctx.fillRect(
              node.x - bckgDimensions[0] / 2,
              node.y + (node.size || 6) + 2,
              bckgDimensions[0],
              bckgDimensions[1]
            );

            // Draw label text
            ctx.textAlign = "center";
            ctx.textBaseline = "middle";
            ctx.fillStyle = "#333";
            ctx.fillText(
              label,
              node.x,
              node.y + (node.size || 6) + 2 + bckgDimensions[1] / 2
            );
          }}
          linkColor={(link: any) => link.color || "#999"}
          linkWidth={(link: any) => link.width || 1}
          linkDirectionalParticles={2}
          linkDirectionalParticleSpeed={0.01}
          onNodeClick={handleNodeClick}
          onNodeRightClick={handleNodeRightClick}
          cooldownTicks={100}
          onEngineStop={() =>
            graphRef.current && graphRef.current.zoomToFit(400)
          }
        />
      </div>

      {/* Node Editor */}
      {selectedNode && (
        <div className="bg-gray-50 dark:bg-dark-secondary border-t dark:border-dark-divider p-3">
          <div className="text-sm font-medium mb-2 dark:text-dark-textPrimary">
            Edit Node:{" "}
            {graphData.nodes.find((n) => n.id === selectedNode)?.name}
          </div>
          <input
            type="text"
            value={
              graphData.nodes.find((n) => n.id === selectedNode)?.name || ""
            }
            onChange={(e) => updateNodeName(selectedNode, e.target.value)}
            className="w-full px-2 py-1 border dark:border-dark-divider rounded text-sm dark:bg-dark-tertiary dark:text-dark-textPrimary"
          />
        </div>
      )}

      <div className="bg-gray-50 dark:bg-dark-secondary border-t dark:border-dark-divider p-2 text-xs text-gray-500 dark:text-gray-400">
        Left click: select • Right click: delete • Drag: move nodes
      </div>
    </div>
  );
}
